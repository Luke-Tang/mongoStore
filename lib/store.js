...
class MongoStore extends Store {
	...
	create(req, cb) {
		...
		if (req.data instanceof Array) {
			collection.insertMany(req.data, this.options.store.create, function (err, resp) {
				if (err) {
					return cb(err)
				}
				const result = {
			...
			collection.insertOne(req.data, this.options.store.create, function (err, resp) {
				if (err) {
					return cb(err)
				}
				const result = {
	...
	remove(req, cb) {
		const collection = this._driver.collection(req.collection)
		collection.deleteMany(req.query, this.options.store.remove, function (err, resp) {
			if (err) {
				return cb(err)
			}
			const result = {
	...
	update(req, data, cb) {
		const collection = this._driver.collection(req.collection)
		collection.findOneAndUpdate(req.query, data, this.options.store.update, function (err, resp) {
			if (err) {
				return cb(err)
			}
			const result = resp.value
	...
	replace(req, data, cb) {
		const collection = this._driver.collection(req.collection)
		collection.updateMany(req.query, data, this.options.store.replace,
			function (err, resp) {
				if (err) {
					return cb(err)
				}
